...
**引数は厳格に検査される.** ツールが宣言していない引数を含む呼び出しは、その名前
を挙げて失敗する（`arguments: json: unknown field "offest"`）。従来は無視して実行
していたため、`offset` の綴り間違いは `search_vendor` のどのページ要求も先頭
ページとして返していた。型が違う引数も同様に拒否される。引数のデコードより前には
何も実行しない。引数をまったく渡さない呼び出しは従来どおり「引数なし」として
扱われる。

`search_vendor` はマッチをインラインで、1 ページずつ返す。大手ベンダーは数百の
プレフィクスを保有するため、`limit`（既定 50）が 1 ページの量を決め、`offset` で
残りを辿り、`has_more` が続きの有無を示す。サーバーはファイルを書かず、パス引数も
取らない。
